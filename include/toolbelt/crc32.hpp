#ifndef __TOOLBELT_CRC32_HPP__
#define __TOOLBELT_CRC32_HPP__
...
namespace toolbelt {
    namespace hidden {
        ...
        struct constexpr_string {
            ...
            constexpr constexpr_string(const char (&s)[Length]) : data_(s), size_(Length - 1) {}
        };

        constexpr std::uint32_t crc_recursive(std::uint32_t crc, constexpr_string str) {
            if(*str.data_ == 0) {
                return crc ^ 0xffffffff;
            }

            auto newcrc = precomputed_table[
                static_cast<unsigned char>(crc) ^ static_cast<unsigned char>(*str.data_)] ^ (crc >> 8);
    ...
    constexpr std::uint32_t crc32(hidden::constexpr_string str) {
        return hidden::crc_recursive(0xffffffff, str);
    }
}
